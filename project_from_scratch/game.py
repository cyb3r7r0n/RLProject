...
class SwitchGame:
    ...
    def reset(self):
        ...
        def parralelize(b, step):
            self.active_agent[b][step] = agent_id
            self.has_been[b][step][agent_id - 1] = 1

        Parallel(n_jobs=2, prefer="threads")
        (delayed(parralelize)(b, step) for b in range(self.opt.bs) for step in range(self.opt.nsteps))
        return self
    ...
    def get_action_range(self, step, agent_id):
        ...
        Parallel(n_jobs=2, prefer="threads")
        (delayed(parralelize)(b) for b in range(self.opt.bs))  
    ...
    def get_state(self):
        ...
        Parallel(n_jobs=2, prefer="threads")
        (delayed(parralelize)(b, a) for b in range(self.opt.bs) for a in range(1, self.opt.game_nagents + 1))  
        return state
